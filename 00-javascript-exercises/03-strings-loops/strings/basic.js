...
//     // Ha az első string végén van szóköz, és a második nagybetűvel kezdődik
//   if (str1.slice(-1) === ' ' && str2[0]  === str2[0].toUpperCase()) {
//     // Először nagybetűsre alakítjuk az első karaktert, és összefűzzük               // GIPI
//     return str1.charAt(0).toUpperCase() + str1.slice(1) + str2;
//   }
//   // Egyébként csak összefűzzük őket kis módosítások nélkül
//   return str1 + str2;

    let word = '';
    word = str1 + str2;
    return word;     // vagy return str1 + str2
...
function checkLetters(str) {
   ...
   if (
    firstChar === firstChar.toLowerCase() ||     // J === J.loweCase() j        J === j
...
function replaceFirstDotToAt(str) { 
    return str.replace('.', '@');                   // ha az összes . ot csaerélni akarjuk akkor: replaceAll
}
...
function removeSpaces(str) { 
    ...
    // return str.split (" ").join('');     // Eszti ver.
...
function getFamilyName(fullName) { 
    // const nev = fullName.split(' ');    // Eszti verzio
    // if (nev.length == 2) {
    // return nev[1];
    // } else {
    // return 'invalid name';
    ...
    // function getFamilyName(fullName) {                                     //#2
